...
const ATTESTATION_SCHEMA = "reallyme.platform.crates_preflight.v1";
...
const readAttestation = (path) => {
  ...
  let contents;
  try {
    fileStatus = lstatSync(path);
    if (
      fileStatus.isSymbolicLink() ||
      !fileStatus.isFile() ||
      fileStatus.size === 0 ||
      fileStatus.size > MAX_ATTESTATION_BYTES
    ) {
...
const resolvePreflightRun = ({ repository, releaseSha, releaseVersion }) => {
  ...
  const runs = runJson(
    [
      "api",
      `repos/${repository}/actions/workflows/${workflow.id}/runs` +
...
export const verifyReleaseAttestation = ({ env = process.env } = {}) => {
  ...
  if (env.RELEASE_ATTESTATION_RESOLVE_ONLY === "1") {
    return { releaseSha, releaseVersion, runId };
  }
  const workflowRun = runJson(
...
if (isMain) {
  try {
    ...
    const code =
      error instanceof ReleaseAttestationError ? error.code : "unexpected-failure";
