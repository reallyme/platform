...
const MAX_CADVISOR_METRICS_BYTES: usize = 2 * 1024 * 1024;

/// Minimal cAdvisor signal used by the agent.
#[derive(Debug, Clone, Copy, PartialEq, Eq)]
pub struct CadvisorSnapshot {
    container_metric_lines: u32,
    has_cpu_metrics: bool,
    has_memory_metrics: bool,
}

impl CadvisorSnapshot {
    /// Returns the number of cAdvisor metric lines referencing containers.
...
/// Fetches and validates the local cAdvisor Prometheus endpoint.
pub async fn collect_cadvisor_metrics(
    url: &Url,
    request_timeout: Duration,
) -> AgentResult<CadvisorSnapshot> {
    let mut response = shared_http_client()?
        ...
        .await
        .map_err(|_error| {
            HephaestusAgentError::new(HephaestusAgentErrorReason::CadvisorUnavailable)
        })?;
    if !response.status().is_success() {
        return Err(HephaestusAgentError::new(
            HephaestusAgentErrorReason::CadvisorUnavailable,
        ));
    }
    let body = read_limited_metrics_body(
        &mut response,
        MAX_CADVISOR_METRICS_BYTES,
        HephaestusAgentErrorReason::CadvisorUnavailable,
    )
    .await?;
    ...
    Ok(parse_cadvisor_metrics(text))
}
...
fn parse_cadvisor_metrics(text: &str) -> CadvisorSnapshot {
    let mut container_metric_lines = 0u32;
    let mut has_cpu_metrics = false;
    let mut has_memory_metrics = false;
    for line in text.lines() {
        if !line.starts_with("container_") {
            continue;
        }
        container_metric_lines = container_metric_lines.saturating_add(1);
        if !has_cpu_metrics && line.starts_with("container_cpu_usage_seconds_total") {
            has_cpu_metrics = true;
        }
        if !has_memory_metrics && line.starts_with("container_memory_usage_bytes") {
