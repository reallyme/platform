...
The public repository is organized around reusable kits and reference hosts:

```text
platform/
├── crates/
│   └── platform/         # The reallyme-platform facade crate
├── kits/
│   ├── app/
│   ├── server/
│   ├── foundationdb/
...
The root is a virtual Cargo workspace. `crates/platform` contains the thin
`reallyme-platform` facade. It always re-exports the host-neutral application
kit and exposes the native server kit and infrastructure kits through opt-in
features. Its default feature set is empty. It does not wrap or replace
underlying crate types, and each crate remains independently usable. Product
crates remain direct dependencies of their consumers.
