...
behind Cargo `build.rs` or commit generated Rust output. The standard Connect generation path uses
`protoc-gen-buffa` for message/view types, `protoc-gen-connect-rust` for
Connect service stubs, and `protoc-gen-buffa-packaging` for module trees.
...
New product apps should start from `apps/reallyme-template-app`, which shows
how to use app-kit contracts while keeping runtime/server lifecycle in host
adapters.
...
- app core depends on `reallyme-app-kit`, `reallyme-domain`, and app-owned ports
...
- product/domain semantics stay in app crates or `reallyme-domain`
